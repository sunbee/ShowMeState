...
void senseTouch() {
  ...
  for (uint8_t b = 0; b < 15; b++) {
    ...
    if (key[b].justPressed()) {
      ...
      _tft.setTextDatum(TL_DATUM);        // Use top left corner as text coord datum
      _tft.setFreeFont(&FreeSans18pt7b);  // Choose a nicefont that fits box
      _tft.setTextColor(DISP_TCOLOR);     // Set the font colour

      // Draw the string, the value returned is the width in pixels
      int xwidth = _tft.drawString(numberBuffer, DISP_X + 4, DISP_Y + 12);

      // Now cover up the rest of the line up by drawing a black rectangle.  No flicker this way
      // but it will not work with italic or oblique fonts due to character overlap.
      _tft.fillRect(DISP_X + 4 + xwidth, DISP_Y + 1, DISP_W - xwidth - 5, DISP_H - 2, TFT_BLACK);
